<?php
   ...
   $section_title = 'About Why Choose Us';
   ...
   $breadcumb_title = 'Add '.$section_title;
    ...
    if(isset($_GET['id']) && $_GET['id']!=''){
        ...
        if($check>0){
         ...
         <script>
        window.location.href='about_why_choose_us.php';
        </script>
        <?php
            die();
        }
        ...
        if($msg==''){
            ...
            if($_FILES['image']['name']!=''){
                    $image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
                     if (!move_uploaded_file($_FILES['image']['tmp_name'], WHY_IMAGE_SERVER_PATH . $image)) {
                        throw new Exception("Failed to move the uploaded file.");
                    }else{
                        move_uploaded_file($_FILES['image']['tmp_name'],WHY_IMAGE_SERVER_PATH.$image);
                    }
                ...
                <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data" class="needs-validation tooltip-label-right" >
                              ...
                              if ($image!='') {
                              	echo "<a target='_blank' href='".WHY_IMAGE_SITE_PATH.$image."'><img width='100px' height='100px' src='".WHY_IMAGE_SITE_PATH.$image."'/></a>";
                              }
									 ?>
