        ...
        if($msg==''){
            ...
            if($_FILES['image']['name']!=''){
                    $image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
                     if (!move_uploaded_file($_FILES['image']['tmp_name'], WEBINAR_IMAGE_SERVER_PATH . $image)) {
                        throw new Exception("Failed to move the uploaded file.");
                    }else{
                        move_uploaded_file($_FILES['image']['tmp_name'],WEBINAR_IMAGE_SERVER_PATH.$image);
                    }
                    mysqli_query($con,"update $table set title='$title',date='$date',time='$time',location='$location',,description='$description',video='$video',image='$image', status=$status,popular='$popular' where id='$id'");
                }
                else{
                    mysqli_query($con,"update $table set title='$title',date='$date',time='$time',location='$location',description='$description',video='$video', status=$status,popular='$popular'  where id='$id'");
                }
                ...
                mysqli_query($con,"insert into $table(title,date,time,location,description,image,video,status,popular) values('$title','$date','$time','$location','$description','$image','$video','$status','$popular')");
            }
        //	header('location:banner.php');
         ...
         <script>
        window.location.href='webinar_list.php';
        </script>
        <?php
            die();
        }
                ...
                <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data" class="needs-validation tooltip-label-right" >
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Webinar Date</label>
                                    <div class="col-sm-10">
                                        <input type="date" class="form-control" id="" name="date" value="<?php echo $date?>">                                    
                                    </div>
                                </div> 
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Webinar Time</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="" name="time" value="<?php echo $time?>">                                    
                                    </div>
                                </div> 
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Webinar Location</label>
                                    <div class="col-sm-10">
                              ...
                              if ($image!='') {
                              	echo "<a target='_blank' href='".WEBINAR_IMAGE_SITE_PATH.$image."'><img width='100px' height='100px' src='".WEBINAR_IMAGE_SITE_PATH.$image."'/></a>";
                              }
									 ?>
                                ...
                                <fieldset class="form-group">
                                    <div class="row">
                                        <label class="col-form-label col-sm-2 pt-0">Popular Webinar</label>
                                        <div class="col-sm-10">
                                            <div class="form-check">
